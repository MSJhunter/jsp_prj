...
$(function(){
  ...
  function movePage(flag) {
	//<form>태그의 action을 변경 
	//document.폼이름.action = "변경할action"
	//<form>태그의 method을 변경
	//document.폼이름.method = "변경할	요청방식 GET|POST"
	...
	//위 그냥 삭제하는게 아님. 다른 사람의 글을 삭제할 수 있음. 
	
	//1. 변경은 내용이 존재하는지 판단
	//summernote <p><br></p>를 가짐.
	//alert($("#contetnt").val().length )
	
	if(flag == 'u') {
			if($("#content").val().length < 15){
				alert("내용은 필수 입력 입니다."); 
				return; 
	//trim 해주기 -> 서브노트는 기본적으로 앞에 p 가짐			
...
<div id="contentDiv">
<div id="writeFrmDiv" >
<form name="readFrm" id="readFrm" method="post"> <!-- action 없으면 나에게 요청 -->
	<table>
	<tr>
